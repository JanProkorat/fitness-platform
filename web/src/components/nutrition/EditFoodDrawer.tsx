...
export default function EditFoodDrawer({ food, onSaved, onClose }: EditFoodDrawerProps) {
  ...
  return (
    <div>
      ...
      <div className="mb-4 rounded-sm border border-border bg-surface px-3 py-2">
        <span className="text-xs capitalize text-text3">{food.source ?? 'custom'}</span>
      </div>
      ...
      <form onSubmit={handleSubmit((data) => mutation.mutate(data))} className="space-y-3">
        ...
        <button
          ...
          className="mt-2 w-full rounded-sm bg-gold px-5 py-2.5 font-heading text-xs font-bold uppercase tracking-wide text-black transition-colors hover:bg-gold-bright disabled:opacity-50"
        >
