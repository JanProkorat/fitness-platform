...
/// <summary>
/// Testcontainers integration tests (real PostgreSQL + MongoDB) for the training-plan-template
/// sharing library (#862) — the read/write/read-guarded-write visibility matrix across the three
/// guard classes (<c>GET</c>/search, <c>PUT</c>/<c>DELETE</c>, <c>copy</c>), the Trainer role
/// gate, hard-delete semantics, and server-computed <c>WeekCount</c>.
/// </summary>
[Collection(TestCollection.Name)]
public class TrainingPlanTemplateEndpointTests(FitnessApiFactory factory)
{
    private static string UniqueEmail(string tag) => $"{Guid.NewGuid():N}@training-plan-templates-{tag}.com";
    ...
    {
        var (_, ownerId) = await RegisterAsync("Trainer", tag);
        return ownerId;
    }
    ...
    private static TrainingPlanTemplate BuildTemplate(
        Guid ownerId, LibraryVisibility visibility = LibraryVisibility.Private, int weekCount = 1) =>
        new TrainingPlanTemplateBuilder()
            .WithOwnerId(ownerId)
            .WithVisibility(visibility)
            .WithWeekCount(weekCount)
            .Build();
    ...
    {
        var (nutritionist, _) = await RegisterAsync("Nutritionist", "nutritionist-search");

        var response = await nutritionist.GetAsync("/training/plan-templates");

        response.StatusCode.Should().Be(HttpStatusCode.Forbidden);
    }

    [Fact]
    public async Task GetTemplate_NutritionistRoleOnPublicTemplate_Returns403()
    {
        ...
        var (nutritionist, _) = await RegisterAsync("Nutritionist", "nutritionist-get");

        var response = await nutritionist.GetAsync($"/training/plan-templates/{template.ExternalId}");
    ...
    {
        var (owner, ownerId) = await RegisterAsync("Trainer", "owner-get-private");
        var template = BuildTemplate(ownerId);
        await SeedTemplateAsync(template);

        var response = await owner.GetAsync($"/training/plan-templates/{template.ExternalId}");
    ...
    public async Task GetTemplate_OtherOwnersPublic_Returns200()
    {
        ...
        var (caller, _) = await RegisterAsync("Trainer", "caller-read-public");

        var response = await caller.GetAsync($"/training/plan-templates/{template.ExternalId}");
    ...
    public async Task GetTemplate_OtherOwnersPrivate_Returns404WithNotFoundCode()
    {
        ...
        var (caller, _) = await RegisterAsync("Trainer", "caller-read-private");
    ...
    {
        var (caller, _) = await RegisterAsync("Trainer", "caller-missing");

        var response = await caller.GetAsync($"/training/plan-templates/{Guid.NewGuid()}");
    ...
    public async Task Put_OtherOwnersPublic_Returns403NotOwned()
    {
        ...
        var (caller, _) = await RegisterAsync("Trainer", "caller-put-public");

        var response = await caller.PutAsJsonAsync(
            $"/training/plan-templates/{template.ExternalId}",
            new
            {
                Name = "Hijacked",
        ...
        response.StatusCode.Should().Be(HttpStatusCode.Forbidden);
        var body = await response.Content.ReadAsStringAsync(TestContext.Current.CancellationToken);
        body.Should().Contain("TRAINING_PLAN_TEMPLATE_NOT_OWNED");
    }

    /// <summary>
    /// The load-bearing test named explicitly in issue #862: PUT on another owner's Private
    ...
    public async Task Put_OtherOwnersPrivate_StaleVersion_Returns404NotVersionConflict()
    {
        ...
        var (caller, _) = await RegisterAsync("Trainer", "caller-put-private-stale");
    ...
    {
        var (owner, ownerId) = await RegisterAsync("Trainer", "owner-put-own-stale");
        var template = BuildTemplate(ownerId, LibraryVisibility.Private);
        await SeedTemplateAsync(template);
        ...
        var body = await response.Content.ReadAsStringAsync(TestContext.Current.CancellationToken);
        body.Should().Contain("TRAINING_PLAN_TEMPLATE_VERSION_CONFLICT");
    }
    ...
    public async Task Delete_OtherOwnersPrivate_Returns404()
    {
        ...
        var (caller, _) = await RegisterAsync("Trainer", "caller-delete-private");

        var response = await caller.DeleteAsync($"/training/plan-templates/{template.ExternalId}");
    ...
    {
        var (owner, ownerId) = await RegisterAsync("Trainer", "owner-delete-own");
        var template = BuildTemplate(ownerId, LibraryVisibility.Private);
        await SeedTemplateAsync(template);

        var response = await owner.DeleteAsync($"/training/plan-templates/{template.ExternalId}");
    ...
    public async Task Copy_OtherOwnersPrivate_Returns404()
    {
        ...
        var (caller, _) = await RegisterAsync("Trainer", "caller-copy-private");
    ...
    {
        var (owner, ownerId) = await RegisterAsync("Trainer", "owner-search");
        ...
        var response = await owner.GetAsync("/training/plan-templates?pageSize=100");
        response.StatusCode.Should().Be(HttpStatusCode.OK);
    ...
    {
        var (owner, _) = await RegisterAsync("Trainer", "owner-create-weekcount");

        var response = await owner.PostAsJsonAsync("/training/plan-templates", new
        {
    ...
    {
        var (owner, _) = await RegisterAsync("Trainer", "owner-create-mutex");
    ...
    // ── create with a genuinely populated week tree ──────────────────────────

    [Fact]
    public async Task CreateTemplate_WithPopulatedWeekTree_PersistsFullShapeWithFreshIds()
    {
        var (owner, _) = await RegisterAsync("Trainer", "owner-create-populated");
        ...
        session.SessionId.Should().NotBeEmpty("the server mints a fresh SessionId when the request omits one");

        var exercise = session.StandaloneExercises.Should().ContainSingle().Subject;
        exercise.ExerciseExternalId.Should().Be(exerciseExternalId);
        exercise.ExerciseName.Should().Be("Bench Press");
        exercise.ExerciseId.Should().NotBeEmpty("the server mints a fresh ExerciseId when the request omits one");
    }

    [Fact]
    public async Task CreateTemplate_DayOfWeekOutOfRange_Returns400()
    {
        var (owner, _) = await RegisterAsync("Trainer", "owner-create-dow-invalid");
        ...
        var response = await owner.PostAsJsonAsync("/training/plan-templates", new
        {
            ...
            Weeks = new[]
            {
                ...
                {
                    WeekNumber = 1,
                    Days = new[] { new { DayOfWeek = 99, Sessions = Array.Empty<object>() } }
                }
            }
        });
    ...
    [Fact]
    public async Task CreateTemplate_DuplicateOrderAcrossWorkoutsAndStandaloneExercises_Returns400()
    {
        var (owner, _) = await RegisterAsync("Trainer", "owner-create-order-dup");
        ...
        response.StatusCode.Should().Be(HttpStatusCode.BadRequest);
        var body = await response.Content.ReadAsStringAsync(TestContext.Current.CancellationToken);
        body.Should().Contain("TRAINING_DUPLICATE_SESSION_ORDER");
    }
