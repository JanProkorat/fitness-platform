...
/// <summary>
/// Tests for <see cref="SaveMealTemplateFromPlanValidator"/>.
/// </summary>
public class SaveMealTemplateFromPlanValidatorTests
{
    private readonly SaveMealTemplateFromPlanValidator _validator = new();

    private static SaveMealTemplateFromPlanRequest ValidRequest() => new()
    {
        PlanId = Guid.NewGuid(),
        WeekNumber = 1,
        DayOfWeek = 1,
    ...
    [Fact]
    public void MealId_Empty_FailsWithRequiredCode()
    {
        var req = ValidRequest();
        req.MealId = Guid.Empty;

        var result = _validator.TestValidate(req);
        result.ShouldHaveValidationErrorFor(x => x.MealId).WithErrorCode(ErrorCodes.Required);
    }
