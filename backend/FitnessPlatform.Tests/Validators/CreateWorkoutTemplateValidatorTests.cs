...
/// <summary>
/// Tests for <see cref="CreateWorkoutTemplateValidator"/>, covering the format-config rules it
/// now shares with every other call site via <c>TrainingContentRuleSet.ApplyFormatConfigRules</c>
/// (#1024). Before this issue the validator carried a fourth, property-chained copy of that
/// method that produced an empty <c>PropertyName</c> on Linux/x64 (#276) — the WOD mutation rows
/// below prove the shared, whole-object form is wired correctly at both the root (prefix
/// "Workout") and per-exercise (prefix "Exercise") call sites.
/// </summary>
public class CreateWorkoutTemplateValidatorTests
{
    private readonly CreateWorkoutTemplateValidator _validator = new();
    ...
    private static CreateWorkoutTemplateRequest ValidRequest() => new()
    {
        Name = "Push Day",
    ...
    public void FormatConfig_Mutation_RejectsWithExpectedCode(
        string name, Func<CreateWorkoutTemplateRequest> buildInvalid, string expectedCode, string? expectedName)
    {
    ...
    {
        object[] Row(string name, Func<CreateWorkoutTemplateRequest> build, string code, string pinnedName) =>
