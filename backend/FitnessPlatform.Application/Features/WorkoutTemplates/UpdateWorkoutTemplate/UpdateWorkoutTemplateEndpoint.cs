    ...
    : Endpoint<UpdateWorkoutTemplateRequest, WorkoutTemplateResponse>
{
    ...
    public override async Task HandleAsync(UpdateWorkoutTemplateRequest req, CancellationToken ct)
    {
        ...
        var versionFilter = Builders<WorkoutTemplate>.Filter.Eq(t => t.ExternalId, req.TemplateId)
                            & Builders<WorkoutTemplate>.Filter.Eq(t => t.Version, req.Version);

        var result = await mongo.WorkoutTemplates.ReplaceOneAsync(versionFilter, template, cancellationToken: ct);
