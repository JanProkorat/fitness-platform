...
WHAT DOES IT DO?
  ...
  2. Allows a specific allowlist of safe, mostly read-only commands, plus
     the narrow slice of the React/TS toolchain needed to re-run
     verification fresh (`npm run build`/`npm test`/`npm run lint`,
     `npx tsc`/`npx eslint`, bare `tsc`/`eslint`) — mirroring the dotnet
     pack's reviewer, which allows only `dotnet build/test/format`.
...
ALLOW_RE = re.compile(
    ...
    r"npm (run build|run test|run lint|test)|"
    r"npx (tsc|eslint)|tsc|eslint|"
...
def main() -> int:
    ...
    return deny(
        ...
        "npm run build/run test/run lint/test, npx tsc/eslint, tsc, eslint, "
