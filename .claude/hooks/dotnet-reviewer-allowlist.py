...
WHEN DOES THIS RUN?
  ...
  builds that have side effects beyond inspecting the codebase.
...
WHAT DOES IT DO?
  ...
  2. Allows a specific allowlist of safe, mostly read-only commands (plus
     `dotnet build/test/format`, needed to re-run verification fresh).
...
Ported 1:1 from reviewer-bash-allowlist.sh (an earlier project's .claude/hooks).
"""
...
ALLOW_RE = re.compile(
    r"^(git (status|diff|log|show)|dotnet (build|test|format)|wc|ls|cat|head|tail|pwd|grep|find)( .*)?$"
)
...
def main() -> int:
    ...
    return deny(
        ...
        "dotnet build/test/format, wc, ls, cat, head, tail, pwd, grep, find).",
    )
