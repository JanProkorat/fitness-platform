...
commands (no writes, no builds, no network, no npm/expo toolchain).
...
WHAT DOES IT DO?
  ...
  patterns. It never builds, never runs the React Native/Expo toolchain,
  never stages files. This hook enforces that — no `npm`/`npx`/`node`/
  `expo`/`eslint`/`tsc` in the allowlist, mirroring the dotnet/react packs'
  designer (which likewise excludes their own toolchains).
...
OUTPUT
  ...
  `deny-subagent-merge.py`) and the dotnet/react packs' `designer-allowlist.py`.
"""
