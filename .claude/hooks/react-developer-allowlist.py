...
for the `developer` common agent. Restricts Bash to the React/TS toolchain,
git, and read-only file inspection commands only.
...
WHAT DOES IT DO?
  ...
  2. Allows the React/TS toolchain (`npm`, `npx`, `node`, `eslint`, `tsc`,
     `vite`) plus `git status/diff/log/show/add/stash/checkout` and
...
ALLOW_RE = re.compile(
    r"^(npm|npx|node|eslint|tsc|vite|"
...
def main() -> int:
    ...
    return deny(
        ...
        f"developer: '{cmd}' not in allowlist. Allowed: npm, npx, node, eslint, tsc, vite, "
