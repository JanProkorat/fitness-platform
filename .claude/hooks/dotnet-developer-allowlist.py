...
for the `developer` common agent. Restricts Bash to dotnet, git, and
read-only file inspection commands only.
...
WHAT DOES IT DO?
  ...
  2. Allows a specific set of safe commands needed for TDD workflows
     (`dotnet ...`, `git status/diff/log/show/add/stash/checkout`, and
     read-only file inspection).
  3. Rejects everything else (curl, python3, node, wget, etc.).
...
OUTPUT
  ...
  (`split-compound-commands.py`, `deny-subagent-merge.py`).

Ported 1:1 from developer-bash-allowlist.sh (an earlier project's .claude/hooks).
"""
...
ALLOW_RE = re.compile(
    r"^(dotnet|git (status|diff|log|show|add|stash|checkout)|ls|cat|head|tail|wc|pwd|grep|find)( .*)?$"
)
...
def main() -> int:
    ...
    return deny(
        ...
        f"developer: '{cmd}' not in allowlist. Allowed: dotnet, "
