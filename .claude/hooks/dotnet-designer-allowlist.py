...
commands (no writes, no builds, no network).
...
WHAT DOES IT DO?
  ...
  patterns. It never builds, never runs tests, never stages files. This hook
  enforces that.
...
OUTPUT
  ...
  `deny-subagent-merge.py`).

Ported 1:1 from designer-bash-allowlist.sh (an earlier project's .claude/hooks).
"""
