...
function NutritionPlanCard({
  ...
  // TODO: Replace static values once API endpoints are implemented
  const daysCompleted = 22
  const compliancePercent = 95
  const weightProgress = '−2,4'
      ...
      <View style={[styles.planCard, { backgroundColor: colors.bg2 }]}>
        ...
        <View style={styles.statsRow}>
          ...
          <View style={styles.statItem}>
            ...
            <Text style={[styles.statDesc, { color: colors.label3 }]}>{t('plans.kgProgress')}</Text>
          </View>
        </View>
              ...
              {hasNutritionPlan && nutritionQuery.data && (
                <View style={styles.cardWrap}>
                  <NutritionPlanCard
                    ...
                    onPress={() =>
                      router.push('/(client)/nutrition/plan-detail' as never)
                    }
                  />
                </View>
              )}
                  ...
                  <View key={plan.planId} style={styles.cardWrap}>
                    <CompletedPlanCard
                      ...
                      onPress={() =>
                        router.push({
                          pathname: '/(client)/plans/[planId]',
                          params: { planId: plan.planId, type: plan.type },
                        } as never)
                      }
                    />
                  </View>
