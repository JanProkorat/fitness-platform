...
import { MovementType } from '@/api/training'
 ...
 *
 * Mirrors web/src/lib/training-plan-format.ts so both clients show the same
 * derived totals on session/plan summaries.
 */
 ...
 * the exercise-card summary line. Mirrors web/src/lib/training-plan-format.ts
 * so both clients render identical summaries.
 */
...
export function formatExerciseSummary(
  ...
  switch (mt) {
    ...
    case MovementType.RepsForTime:
      return `${withCount(reps)} · ${duration}`
    case MovementType.Reps:
    default:
