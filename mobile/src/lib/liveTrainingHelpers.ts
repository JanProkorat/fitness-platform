...
/**
 * Actual reps/weight/duration/distance logged for a set, as recorded by the
 * (now-deleted) live-session store. Defined locally so this module has no
 * dependency on any UI-layer store — the shape is preserved verbatim so a
 * future live-session store can re-adopt it without drift.
 */
